...
public class WorkflowParser {
     ...
     */
    private static void setDepth(List<Job> jobList) {
        for (Job job : jobList) {
            job.setDepth(0);
        }

        Map<Job, Integer> inDegreeMap = new HashMap<>();
        for (Job job : jobList) {
            inDegreeMap.put(job, job.getParentList().size());
        }

        Queue<Job> queue = new LinkedList<>();
        for (Job job : jobList) {
            if (inDegreeMap.get(job) == 0) {
                job.setDepth(0);
                queue.offer(job);
            }
        }
        ...
        while (!queue.isEmpty()) {
            ...
            for (Job child : current.getChildList()) {
                int childDepth = child.getDepth();
                if (currentDepth + 1 > childDepth) {
                    child.setDepth(currentDepth + 1);
                }

                int inDegree = inDegreeMap.get(child) - 1;
